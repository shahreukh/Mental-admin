...
<div class="container-fluid px-4">
    ...
    <div class="card mt-4">
        <div class="card-header">
            <h4>Add Post
                <a href="{{ url('admin/add-corpost')}}" class="btn btn-primary float-end">Add Post</a>
            </h4>
        </div>
        <div class="card-body">
            ...
            <form action="{{ url('admin/add-corpost')}}" method="POST">
                @csrf

                <div class="mb-3">
                    <label for="">Corporate</label>
                    <select name="corporate_id" class="form-control">
                        @foreach ($corporate as $corpost)
                            <option value="{{ $corpost->id}}">{{ $corpost->name}}</option>
                        @endforeach
                    </select>
                </div>
